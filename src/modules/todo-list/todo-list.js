...
class TodoList {
  ...
  init(targetNode) {
    const markup = targetNode || document.body;
    markup.innerHTML = this.#appMarkup;
  ...
  #getRefs() {
    this.#refs.list = document.querySelector('.todo-list');
    if (this.#refs.list) {
      this.#refs.addItemBtn = this.#refs.list.querySelector('.header__button');
      this.#refs.itemInput = this.#refs.list.querySelector('.header__input');
      this.#refs.currentList = this.#refs.list.querySelector('.current-list');
      this.#refs.doneList = this.#refs.list.querySelector('.done-list');
      this.#refs.listContainer = this.#refs.list.querySelector('.list-container');
    }
  }

  #setListener() {
    this.#refs.addItemBtn.addEventListener('click', this.#addObj.bind(this));
    this.#refs.listContainer.addEventListener('click', this.#onClick.bind(this));
  }
  ...
  #onClick(e) {
    ...
    const btnRefs = e.target.closest('.list__item[data-id]');

    if (btnRefs) {
      if (e.target.dataset.action === 'remove') {
        this.#removeTask(btnRefs.dataset.id);
      }
      if (e.target.dataset.action === 'toggle') {
        this.#togleTask(btnRefs.dataset.id);
      }
    }
  }
